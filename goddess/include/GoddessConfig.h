...
class GoddessConfig
{
	private:
		...
		///Read the reaction information. These informations are all in the file share/goddess.reacinfo which is copied in the exec folder upon installation.
		void ReadReacInfo(unsigned int run);
	...
	public:
		///Default constructor.
		GoddessConfig(unsigned int run, std::string positionFile, std::string configFile, std::string sx3EnAdjustFName = "",
				std::string qqq5EnAdjustFName = "");
		///Default destructor.
